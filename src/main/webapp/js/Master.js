     ...
     $('.deleteBoardBtn').on('click', function() {
         ...
         if (confirm("정말로 삭제하시겠습니까?")) {
             $.ajax({
                 ...
                 url: '/master/boardMasterAllDelete/' + idx, // 삭제 요청 URL
                 success: function(response) {
                     alert('해당 ' + idx + ' 번호의 게시글이 삭제되었습니다.'); // 성공 메시지
                     window.location.reload(); // 페이지 새로고침
                 },
                 error: function(xhr) {
     ...
     $('.deleteFaqBtn').on('click', function() {
         ...
         if (confirm("정말로 삭제하시겠습니까?")) {
             $.ajax({
                 ...
                 url: '/master/faqMasterDelete/' + idx, // 삭제 요청 URL
                 success: function(response) {
                     alert('해당 ' + idx + ' 번호의 FAQ가 삭제되었습니다.'); // 성공 메시지
                     window.location.reload(); // 페이지 새로고침
                 },
                 error: function(xhr) {
                     console.error("삭제 중 에러 발생:", xhr.responseText); // 에러 로그
 ...
 $(document).ready(function() {
      $('.deleteReviewBtn').on('click', function() {
          ...
          if (confirm("정말로 삭제하시겠습니까?")) {
              $.ajax({
                  type: 'POST', // HTTP 메서드를 POST로 설정
                  url: '/master/boardMasterReviewDelete/' + idx, // 삭제 요청 URL
                  success: function(response) {
                      alert('해당 ' + idx + ' 번호의 리뷰가 삭제되었습니다.'); // 성공 메시지
                      window.location.reload(); // 페이지 새로고침
                  },
                  error: function(xhr) {
                      console.error("삭제 중 에러 발생:", xhr.responseText); // 에러 로그
  ...
  $(document).ready(function() {
        $('.deleteReplyBtn').on('click', function() {
            ...
            if (confirm("정말로 삭제하시겠습니까?")) {
                $.ajax({
                    type: 'POST', // HTTP 메서드를 POST로 설정
                    url: '/master/boardreplyMasterDelete/' + idx, // 삭제 요청 URL
                    success: function(response) {
                        alert('해당 ' + idx + ' 번호의 댓글이 삭제되었습니다.'); // 성공 메시지
                        window.location.reload(); // 페이지 새로고침
                    },
                    error: function(xhr) {
                        console.error("삭제 중 에러 발생:", xhr.responseText); // 에러 로그
    ...
    $(document).ready(function() {
            $('.deleteNoticeBtn').on('click', function() {
                ...
                if (confirm("정말로 삭제하시겠습니까?")) {
                    $.ajax({
                        type: 'POST', // HTTP 메서드를 POST로 설정
                        url: '/master/noticeMasterListDelete/' + idx, // 삭제 요청 URL
                        success: function(response) {
                            alert('해당 ' + idx + ' 번호의 공지사항이 삭제되었습니다.'); // 성공 메시지
                            window.location.reload(); // 페이지 새로고침
                        },
                        error: function(xhr) {
                            console.error("삭제 중 에러 발생:", xhr.responseText); // 에러 로그
